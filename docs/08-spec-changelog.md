...
## v1.1.8 사진 위치 200m 인증 (2026-09-21)

| 대상 | 구분 | 변경 전 → 변경 후 | 근거 |
| --- | --- | --- | --- |
| `API-PST-002` | 변경 | 기본 LOW 미리보기 → 대표 사진의 `lat`·`lng`·`takenAt`을 일시 처리하여 최종 선택 장소와 200m 거리·30일 기준을 판정 | DEC-20260921-001, PST-022~028 |
| `API-PST-003` | 변경 | 신규 앱이 위치·시각을 전송하지 않음 → 판정용으로만 전송하고 `posts`에는 null로 저장 | DEC-20260921-001, PST-022~028 |
| `API-PLC-003` | 변경 | Google 역지오코딩 위치·명칭 우선 후보 정렬 → 요청한 사진 좌표와 내부 장소 좌표의 거리순 정렬 | DEC-20260921-001 |
| `tier_logs` | 변경 | 정확한 `taken_at` 보관 → `days_since_taken`만 보관 | DEC-20260921-001 |
...
## 사진 인증 기기 내 판정 전환 (2026-09-21)

| 대상 | 구분 | 변경 전 → 변경 후 | 근거 |
| --- | --- | --- | --- |
| `API-PST-002` | 변경 | 서버에 사진 좌표·촬영 시각을 보내 미리보기 → 앱이 사진과 선택 장소를 기기 안에서 비교 | DEC-20260921-001 |
| `API-PST-003` | 변경 | `lat`, `lng`, `takenAt` 전송 → `localTier`, `localWithinRadius`만 전송 | DEC-20260921-001 |
| 게시·등급 로그 | 정정 | 서버가 사진 위치·시각으로 등급을 판정 → 원 값 없이 기기 판정 등급으로 방문·뱃지 처리 | DEC-20260921-001 |
